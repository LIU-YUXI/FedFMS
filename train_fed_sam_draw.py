...
def draw_fundus_result(image,oc_mask,od_mask,filename):
    ...
    # 保存绘制了轮廓线的图像
    cv2.imwrite('/mnt/diskB/lyx/FedSAM/output/fundus1e-4-1/figure/{}-Contours.jpg'.format(filename), image_with_contours)
...
def draw_result(image,oc_mask,filename):
    ...
    # 保存绘制了轮廓线的图像
    cv2.imwrite('/mnt/diskB/lyx/FedSAM/output/{}/figure/{}-Contours.jpg'.format(args.exp,filename), image_with_contours)
