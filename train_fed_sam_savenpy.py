...
from dataloaders.federated_dataloader_copy import Dataset, ToTensor, ProstateDataset
...
if __name__ == "__main__":
    ...
    for client_idx in range(client_num):
        ...
        dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=False,  num_workers=4, pin_memory=True, worker_init_fn=worker_init_fn)
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for client_idx in source_site_idx:
            ...
            for i_batch, sampled_batch in enumerate(dataloader_current):
                ...
                all_image.append(volume_batch)
                all_label.append(label_batch)
                all_pt.append(pt)
                iter_num = iter_num + 1
                if iter_num % display_freq == 0:
                    logging.info('Epoch: [%d] client [%d] iteration [%d / %d]' % \
                ...
                if iter_num % 10000==0:
                    ...
                    # break
            all_image=torch.cat(all_image, dim=0)
            all_pt=torch.cat(all_pt, dim=0)
            all_label=torch.cat(all_label, dim=0)
            save_path = '/mnt/diskB/lyx/FeTS2022_FedDG_1024_npy'
            save_path = os.path.join(save_path,str(client_name[client_idx]))
